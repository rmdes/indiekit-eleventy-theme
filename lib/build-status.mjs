...
/**
 * Synchronous twin of writeBuildStatus — same tolerant read, merge, atomic
 * tmp+rename, never-throws envelope. Needed by eleventy.before: Eleventy's
 * userland AsyncEventEmitter runs listeners in PARALLEL (Promise.all), so an
 * async write suspends at its first await while a later fully-synchronous
 * handler (the OG execFileSync batch loop) blocks the event loop for minutes;
 * the write must complete synchronously to land before that work starts.
 *
 * @param {object} patch - Fields for the new status
 * @param {string} [path] - Override for tests
 * @returns {boolean} true if persisted
 */
export function writeBuildStatusSync(patch, path = BUILD_STATUS_PATH) {
  const tmpPath = `${path}.tmp`;
  try {
    let previous;
    try {
    ...
    try { unlinkSync(tmpPath); } catch { /* tmp never created */ }
    console.warn(`[build-status] Write failed (build continues): ${error.message}`);
    return false;
  }
}
