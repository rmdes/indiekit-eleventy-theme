 ...
 * @param {string} [options.outputDir] - Where optimized files are written. The
 *   Eleventy config ALWAYS passes the build's real output directory
 *   (`eleventyConfig.directories.output`), because `--output` overrides
 *   `dir.output` and a hardcoded `_site` writes into the wrong directory the
 *   moment a build targets a release directory instead of the live site. The
 *   default here only serves theme-only dev runs with no config override.
 * @returns {Promise<string>} HTML string
 */
export async function renderAvatar(src, attributes = {}, options = {}) {
  const attrs = { decoding: "async", loading: "lazy", ...attributes };
  const imgOutputDir = options.outputDir || "./_site/img/";
