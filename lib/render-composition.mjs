...
async function renderSection(node, renderFn, ctx) {
  ...
  if (catalog?.available) {
    // hasOwn lookup: byId may be a plain object literal (test fixtures), so
    // an inherited name as node.type ("constructor", "toString") must not
    // pass the existence gate and crash in renderFile instead of
    // placeholding. The loader also builds byId with a null prototype.
    const entry = catalog.byId && Object.hasOwn(catalog.byId, node.type)
      ? catalog.byId[node.type]
      : undefined;
    if (!entry) {
