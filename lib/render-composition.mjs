...
async function renderSection(node, renderFn, ctx) {
  ...
  if (catalog?.available) {
    const entry = catalog.byId?.[node.type];
    if (!entry) {
    ...
    if (entry.requiresPlugin && !slug) {
      // Fail open: never let a missing name mapping blank a block.
      console.warn(`[render-composition] no ENDPOINT_SLUGS mapping for "${entry.requiresPlugin}" — rendering "${node.type}" ungated`);
    }
    if (!slug) {
