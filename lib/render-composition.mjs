...
/**
 * Types that exist as BOTH a section template and a widget template (the
 * drift-guard exclusions from KNOWN_WIDGET_TYPES). The Phase-1 flat-
 * vocabulary rule ("section wins") stays the regionless default; inside a
 * complementary region the WIDGET template is the correct render (compact
 * sidebar variant — T4 parity found the section variant visibly regressing
 * the sidebar), so region-aware routing overrides the collision rule there.
 */
export const COLLISION_TYPES = new Set(["recent-posts", "ai-usage"]);
...
export const ENDPOINT_SLUGS = Object.freeze({
  ...
  // Phase 7b — sidebar widgets relocated from site-config BUILTIN_BLOCKS to
  // their owning plugins' get blocks(). Display name (plugin `name` field) →
  // loadout slug (registry key / loaded-plugins.json key).
  "Comments endpoint": "comments",
  "Webmention moderation endpoint": "webmention-io",
  "Blogroll endpoint": "blogroll",
  "ActivityPub endpoint": "activitypub",
  // Phase 7c — github already mapped above; funkwhale owns the "Listening" block.
  "Funkwhale listening activity endpoint": "funkwhale",
});
...
async function renderSection(node, renderFn, ctx) {
  ...
  if (catalog?.available) {
    ...
    if (!slug) {
      ...
      // Known asymmetry (also dies with the maps in Phase 7): map selection is
      // MEMBERSHIP-based, not region-based — COLLISION_TYPES rendered as
      // sidebar widgets still gate via sectionPluginRequirements. Zero live
      // impact: both collision types are plugin-independent in both maps.
