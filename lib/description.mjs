 ...
 *
 * Precedence mirrors what an author would expect: an explicit `description`
 * wins, then a `summary`, then the opening of the post itself. A page with no
 * prose of its own (a listing, an index) yields "" so the caller can fall back
 * to the site description — no page-kind heuristics required.
 *
...
export function deriveDescription(item, limit = 200) {
  ...
  const explicit = data.description || data.summary;
