...
/**
 * True when Eleventy is watching locally rather than serving a deployed site.
 *
 * The run mode alone is not enough. Production does not run one-shot builds:
 * start.sh keeps `eleventy --watch --incremental` alive to serve the site, so
 * ELEVENTY_RUN_MODE is never "build" there either. Keying only on it gave every
 * deployed _data fetch the 4h dev TTL instead of the 15m the callers ask for,
 * and the cache lives in /app/data, which survives container recreation - so a
 * stale entry outlived deploys and restarts. CLOUDRON_APP_ORIGIN exists only in
 * the deployed container, which is the distinction that was missing.
 */
export const isWatchMode = () =>
  process.env.ELEVENTY_RUN_MODE !== "build" && !process.env.CLOUDRON_APP_ORIGIN;
