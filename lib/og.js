 ...
 * root-relative path ("/media/images/me.jpg"), a same-origin URL, or a remote
 * one. Site media is the right home for it: a personal photo in the theme's own
 * images/ is passthrough-copied to every deployment built from this theme.
...
function loadAvatar(src, contentDir) {
  ...
  // A site with no avatar configured gets the theme's neutral silhouette — the
  // same one _data/site.js falls back to for the h-card, so the card and the
  // page agree. It is kept as a PNG alongside the SVG because Satori cannot be
  // trusted to rasterise SVG (see AVATAR_TYPES).
  const path =
    resolveAvatarPath(src, contentDir) ||
    resolve(__dirname, "..", "images", "default-avatar.png");
  if (!existsSync(path)) return avatarDataUri;
...
/**
 * The content-type directory a post lives in ("likes", "replies", ...), or "".
 * @param {string} filePath - Absolute path under content/
 * @returns {string} Directory name, or "" when the path is not under content/
 */
function contentTypeDir(filePath) {
  const parts = String(filePath).split("/");
  const contentIdx = parts.indexOf("content");
  return contentIdx >= 0 && contentIdx + 1 < parts.length - 1 ? parts[contentIdx + 1] : "";
}

/**
 * THE canonical shape of an OG cache key: "<type>-<date>-<slug>".
 *
 * Three call sites derive this key from three different inputs — the source
 * path (toOgSlug, during generation), the output path (the og-fix transform)
 * and the page URL (the `ogSlug` filter, for prev/next nav thumbnails). They
 * MUST agree: whichever disagrees silently stops finding cards.
 *
 * That is not hypothetical. When the type prefix was introduced (2026-09-13)
 * the URL-based derivation was missed, so every prev/next thumbnail on the site
 * lost its image — `hasOgImage` looked for the old flat name, found nothing,
 * and the nav fell through to its text-only branch. Route every derivation
 * through here so there is one definition to change.
 * @param {string} type - Post type, e.g. "likes"
 * @param {string} year
 * @param {string} month
 * @param {string} day
 * @param {string} slug
 * @returns {string}
 */
export function formatOgSlug(type, year, month, day, slug) {
  return `${type}-${year}-${month}-${day}-${slug}`;
}

/**
 * OG cache key from a page URL ("/likes/2026/02/22/5be34/").
 * Non-dated URLs (pages, legacy) fall back to their last segment, which is what
 * the generator produces for content outside a type directory.
 * @param {string} url
 * @returns {string} Slug, or "" for an empty URL
 */
export function ogSlugFromUrl(url) {
  if (!url) return "";
  const segments = String(url).split("/").filter(Boolean);
  if (segments.length === 5) {
    const [type, year, month, day, slug] = segments;
    return formatOgSlug(type, year, month, day, slug);
  }
  return segments[segments.length - 1] || "";
}

/**
 * Cache key / filename for a post's OG card: "<type>-<filename>".
 *
 * The type prefix is NOT decoration. Indiekit slugs are a date plus a short
 * random suffix, so the same basename can legitimately occur under two post
 * types — rmendes has exactly two such pairs, e.g.
 * `likes/2026-02-22-5be34.md` (a like of stefanbohacek.com) and
 * `replies/2026-02-22-5be34.md` (a reply to manton.org). Keyed on the basename
 * alone they collided on ONE cache entry and ONE PNG: each build regenerated
 * both, each overwrote the other's card, and the manifest hash never settled —
 * so every build did two pointless renders forever AND one post served the
 * other's card. Including the type makes the key as unique as the URL is.
 *
 * The og-fix transform derives the same key from outputPath, where the type is
 * the first URL segment.
 * @param {string} filePath - Absolute path under content/
 * @returns {string} Slug, e.g. "likes-2026-02-22-5be34"
 */
function toOgSlug(filePath) {
  const filename = basename(filePath, ".md");
  const type = contentTypeDir(filePath);
...
/**
 * Drop manifest entries for posts that no longer exist.
 *
 * `newManifest` is seeded from the previous manifest so a crash mid-pass cannot
 * lose work (see SAVE_INTERVAL), which also means a deleted post's entry would
 * survive forever — and its PNG with it. rmendes accumulated 62 such entries
 * between March and September.
 *
 * Only prunes on a COMPLETE pass. A batch that stopped at its limit has not
 * looked at the remaining files yet, so anything "unseen" there is merely
 * further down the list, not deleted. Bookkeeping keys (`__default__`) are
 * always kept.
 *
 * @param {object} newManifest - The manifest about to be written
 * @param {Set<string>} seen - Slugs this pass actually examined
 * @param {boolean} hasMore - True when the pass stopped at the batch limit
 * @returns {object} The manifest to persist
 */
function pruneManifest(newManifest, seen, hasMore) {
  if (hasMore) return newManifest;
  return Object.fromEntries(
    Object.entries(newManifest).filter(([key]) => key.startsWith("__") || seen.has(key)),
  );
}
...
export async function generateOgImages(contentDir, cacheDir, config, batchSize = 0) {
  ...
  // Every slug this pass saw, generated or skipped. Used at the end of a
  // COMPLETE pass to drop entries for posts that no longer exist — newManifest
  // is seeded from the previous manifest (for crash resilience, see
  // SAVE_INTERVAL below), so without this a deleted post's entry survives
  // forever. rmendes had accumulated 62 such entries between March and
  // September, each keeping an orphan PNG alive in the cache and the output.
  const seen = new Set();
  ...
  // On a COMPLETE pass (the loop ran out of files rather than hitting the batch
  // limit) every valid slug is in `seen`, so anything else in the manifest
  // belongs to a post that no longer exists. Drop those, and the caller's
  // prune removes their PNGs.
  //
  // Only on a complete pass: a partial one has not looked at the remaining
  // files yet, and pruning against it would delete cards that are merely
  // further down the list. Bookkeeping keys (`__default__`) are kept.
  const manifestToWrite = pruneManifest(newManifest, seen, hasMore);
  const dropped = Object.keys(newManifest).length - Object.keys(manifestToWrite).length;
  if (dropped > 0) {
    console.log(`[og] Dropped ${dropped} manifest entr${dropped === 1 ? "y" : "ies"} for posts that no longer exist`);
  }

  if (hasGC) global.gc();
  writeFileSync(manifestPath, JSON.stringify(manifestToWrite, null, 2));
