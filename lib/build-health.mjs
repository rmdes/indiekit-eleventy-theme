...
/**
 * OUTSIDE the site output, and that is load-bearing.
 *
 * This used to live at /app/data/site/health/build.json — inside the tree
 * Eleventy writes. That is survivable only while the build happens IN PLACE.
 * As soon as a build targets a staging directory that is swapped in on success,
 * a health file written inside the output is either (a) written into the old
 * release and then thrown away by the swap, or (b) written into the new release
 * and therefore reset to nothing on every deploy — and the one state it must
 * report accurately is "the last build did not happen", which is exactly when
 * no new release exists to carry it.
 *
 * So it lives beside build-status.json in persistent app data, and nginx serves
 * it with an `alias`. Same reasoning as build-status.json's own comment: status
 * about the build must not be stored in the build's own product.
 */
export const BUILD_HEALTH_PATH = "/app/data/health/build.json";
