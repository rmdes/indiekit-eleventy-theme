...
export function ogDescription(content, len = 200) {
  ...
  // Force a FLAT copy of the excerpt.
  //
  // V8 represents `slice()` as a SlicedString and `+` as a ConsString: both
  // keep a pointer to the parent instead of copying. The parent here is the
  // page's entire rendered HTML (~230 KB), and Eleventy's watch mode holds the
  // resulting excerpt per page — so a ~200-character description can pin a
  // whole page's HTML that would otherwise be collected after rendering.
  // Across ~3,400 pages that is hundreds of MB of retention for a few hundred
  // KB of actual text. Buffer round-trip materialises a standalone string.
