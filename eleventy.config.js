...
export default function (eleventyConfig) {
  ...
  // NO passthrough for .cache/og.
  //
  // `addPassthroughCopy({ ".cache/og": "og" })` copied the ENTIRE build cache
  // into the output on every build — 2,772 files / 222 MB, and the bulk of the
  // "Copied 5330" line. Two problems with that:
  //
  //  1. It republished bytes that had not changed, on every build, which is what
  //     made a release directory expensive to create from empty and therefore
  //     blocked the atomic release swap.
  //  2. It copied the cache WHOLESALE, including manifest.json — so
  //     /og/manifest.json was publicly served, exposing every card's slug and
  //     title (drafts and deleted posts included, until the next prune).
  //     Verified 200 on rmendes before this change.
  //
  // The eleventy.before hook already mirrors the cards into OG_PUBLIC_DIR, and
  // it copies `*.png` ONLY, so the manifest stays private. That hook is also
  // mtime-aware, so a card regenerated because branding changed still reaches
  // the public dir.
